...
int main()
{
    ...
    float mirrorVertices[] = { // vertex attributes for a quad that fills the entire screen in Normalized Device Coordinates.
        // positions   // texCoords
        -0.2f,  0.1f, 0.0f, 1.0f,
        -0.2f, -0.1f, 0.0f, 0.0f,
         0.2f, -0.1f, 1.0f, 0.0f,
    ...
    // -------------------------
    unsigned int mirrorWidth = 400;
    unsigned int mirrorHeight = 150;
    unsigned int framebuffer;
    glGenFramebuffers(1, &framebuffer);
    glBindFramebuffer(GL_FRAMEBUFFER, framebuffer);
    // create 1 texture buffer for color buffer
    unsigned int textureColorbuffer;
    glGenTextures(1, &textureColorbuffer);
    glBindTexture(GL_TEXTURE_2D, textureColorbuffer);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, mirrorWidth, mirrorHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL); // uninitlaized texture; no data
   ...
   // now that we actually created the framebuffer and added all attachments we want to check if it is actually complete now
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::mat4 projection = glm::perspective(glm::radians(camera.Zoom), (float)mirrorWidth / (float)mirrorHeight, 0.1f, 100.0f);
